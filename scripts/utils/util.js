...
const deployUpgradableContractFirstTimeByWallet = async (
  ...
  storageArgs.push(_proxy_admin)
  const storageContract = await deployContractByWallet(wallet, factoryPathStorage, ...storageArgs)
...
const generateSignatures = (msgHash, wallets) => {
  ...
  for (let i = 0; i < wallets.length; i++) {
    ...
    const { v, r, s } = ecsign(Buffer.from(msgHash.slice(2), "hex"), Buffer.from(wallet.privateKey.slice(2), "hex"))
